    ...
    def importData(self, filename=None):
        ...
        with open(filename) as csvfile:
            ...
            for row in mylist[1:]:
                ...
                blackmarket = False
                if row[fields.index("blackmarket")].upper() == "Y":
                    blackmarket = True

                padSize = row[fields.index("max_pad_size")].upper()

                market = False
                if row[fields.index("market")].upper() == "Y":
                    market = True

                shipyard = False
                if row[fields.index("shipyard")].upper() == "Y":
                    shipyard = True
                
                modifydate = datetime.strptime(row[fields.index("modified")].lower(), "%Y-%m-%d %H:%M:%S")

                outfitting = False
                if row[fields.index("outfitting")].upper() == "Y":
                    outfitting = True

                rearm = False
                if row[fields.index("rearm")].upper() == "Y":
                    rearm = True

                refuel = False
                if row[fields.index("refuel")].upper() == "Y":
                    refuel = True

                repair = False
                if row[fields.index("repair")].upper() == "Y":
                    repair = True
